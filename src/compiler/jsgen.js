...
class JSGenerator {
    ...
    descendInput (block) {
        ...
        case InputOpcode.OP_TO_CASE:
            return `runtime.ext_scratch3_operators.toCase({VALUE: ${this.descendInput(node.value)}, CASE: "${sanitize(node.case)}"})`;
        ...
        } case InputOpcode.SENSING_OF_VISIBILITY: {
            const targetRef = this.descendTargetReference(node.object);
            return `(${targetRef} ? ${targetRef}.visible : 0)`;
